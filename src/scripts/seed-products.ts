...
async function seedProductData() {
  ...
  // 1. Create product categories if they don't exist
  const categories = [
  ...
  const { data: existingCategories, error: categoriesError } = await supabase.from("product_categories").select("*");
  ...
  const products = [
    {
      ...
      price: 12.99,
      stock_quantity: 100,
      category_id: categoryIds[0], // Cleaning Supplies
    },
    {
    ...
    },
    {
      ...
      price: 24.99,
      stock_quantity: 75,
      category_id: categoryIds[1], // Waxes & Polishes
    },
    {
      ...
      price: 14.99,
      stock_quantity: 60,
      category_id: categoryIds[2], // Interior Care
    },
    {
      name: "Tire Shine Gel",
      ...
      price: 9.99,
      stock_quantity: 80,
      category_id: categoryIds[3], // Exterior Care
    },
    {
      ...
      price: 149.99,
      stock_quantity: 15,
      category_id: categoryIds[4], // Tools & Equipment
    },
  ];

  const { data: existingProducts, error: productsError } = await supabase.from("products").select("*");
  ...
  if (existingProducts.length === 0) {
    ...
    productIds = createdProducts.map((prod) => prod.id);
    console.log(`Created ${createdProducts.length} products`);
  } else {
    productIds = existingProducts.map((prod) => prod.id);
    console.log(`Found ${existingProducts.length} existing products`);
  }

  // 3. Check if franchises exist, create sample ones if not
  const { data: existingFranchises, error: franchisesError } = await supabase.from("franchises").select("*");
  ...
  if (existingFranchises.length === 0) {
    console.log("No franchises found. Please create franchises first using the admin panel or seed script.");
    console.log("Skipping order creation for now.");
    return;
  } else {
    franchiseIds = existingFranchises.map((franchise) => franchise.id);
    console.log(`Found ${existingFranchises.length} existing franchises`);
  }
